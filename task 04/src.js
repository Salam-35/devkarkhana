...
function loadTableData(userData) {
    ...
    tableData.innerHTML = userDataHtml;
}

function show(event) {
    let userEmail = event.parentElement.parentElement.children[1].innerText;
    // alert(userEmail)
    let user = [];
    userData.forEach(data => {
    ...
    location.href = `/show.html?user=${user}`;
...
if (searchBtn)
    searchBtn.addEventListener("click", () => {
        let input_data = searchText.value;
        let userDataHtml = '';
        const table = document.getElementById('tableData');
        userData.forEach(data => {

            if (data.email == input_data) {
                userDataHtml += `<tr>
                        <td>${data.firstName}  ${data.lastName}</td>
                        <td>${data.email}</td>
                        <td>${data.point}</td>
...
if (saveUserbtn)
    saveUserbtn.addEventListener("click", () => {
        ...
        const point_ = document.getElementById('point').value;
        const company_ = document.getElementById('company').value;
